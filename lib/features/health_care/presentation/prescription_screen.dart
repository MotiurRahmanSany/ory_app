...
import 'package:ory/features/health_care/presentation/suggestion_screen.dart';
...
class _PrescriptionScreenState extends ConsumerState<PrescriptionScreen> {
  ...
  void _pickImage(bool fromCamera) async {
    final selectedImage =
        fromCamera ? await takeImageWithCamera() : await pickImageFromGallery();
    ...
    return Scaffold(
      appBar: AppBar(
        title: Row(
          ...
          children: [
            const AnimatedAiLogo(isAnimating: false, size: Constants.titleLogoSize),
            const SizedBox(width: 8),
            const Text('Medi Mind'),
          ],
        ),
        centerTitle: true,
        elevation: 0,
